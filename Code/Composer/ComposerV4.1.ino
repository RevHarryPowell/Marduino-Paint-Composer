/*
~~Composer v4.1 changelog~~
*Added funtion noteDuration() to allow note duration to be altered by holding down nd button and turning REA
*Allowed scrolling between octaves without holding down SWA button
*Reversed octave scrolling (with SWA button held) to align with normal vertical scrolling
...
void placeNote(){
  ...
  if (sbIndex > 6 || count < 10){
    ...
    if (Bcount == 14){
      ...
      for (byte g = 0; g < 5; g++){
        if (saveBuffer0[sbIndex][g][0] == REST){
          if (g < f){
            f = g;
          }
        }
        else if (saveBuffer0[sbIndex][g][0] == octave[currentOctave][13-Bcount]){
          f = g + 10;
          break;
        }
      }
      if (f == 6){
        ...
        saveBuffer0[sbIndex][f-10][0] = REST;
      }
    }
  }
  //if the placed beat lies in saveBuffer1
    ...
    if (Bcount == 14){
      ...
      byte f = 6;   //the tone # that is OK to write to (aka lowest unused tone) (6 means all tones in use) (>9 means delete tone #(f-10))    
      for (byte g = 0; g < 5; g++){
        if (saveBuffer1[sbIndex][g][0] == REST){
          if (g < f){
            f = g;
          }
        }
        else if (saveBuffer1[sbIndex][g][0] == octave[currentOctave][13-Bcount]){
          f = g + 10;
          break;
        }
      }
      if (f == 6){
        ...
        saveBuffer1[sbIndex][f-10][0] = REST;
      }
    }
  }
...
void noteDuration(){
  ...
  if((aLast == LOW) && (aState == HIGH)){
    ...
    if(digitalRead(REApinB) == HIGH && noteDur < 48){
      noteDur++;
    }
    sseg2.print(0);
