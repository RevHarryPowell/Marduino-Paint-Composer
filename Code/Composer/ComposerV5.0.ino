/*
~~Composer v5 changelog~~
*placeNote() modified to allow for placement (and deletion) of notes up to 36 beats in length
*OCTup and OCTdown indicators moved to column 15 in matrix
  >corresponds only with that beat only, not entire window

**Modify placeNote() to flash row when long note will overlap with future note of same pitch (& don't place note)
*/
