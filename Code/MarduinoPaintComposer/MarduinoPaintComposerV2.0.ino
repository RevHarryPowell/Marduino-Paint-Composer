/*
~~Marduino Paint Composer v2.0~~
*Playback function encorporated (with LCD menu for choosing song)
*nbPin switch used for changing currentOctave in PLAYBACK mode
*Stop button implemented during playback
*Tempo and current beat are displayed during playback

¡STILL TO DO!
**Play during COMPOSE mode (play while button held down?)
...
void LCDplayList() {
  ...
  //ndPin switch debounce
  if (digitalRead(ndPin) == HIGH){
    dbTime = millis();
  }
  if ((millis() - dbTime) > 40){
    if (digitalRead(ndPin) == LOW){
      if (currentOctave < 6){
        currentOctave++;
        sseg2.writeDigitNum(0, (currentOctave + 1));
        sseg2.writeDisplay();
