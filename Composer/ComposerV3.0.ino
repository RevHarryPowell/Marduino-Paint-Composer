/*
~~Composer v3 changelog~~
*Cursor added at 8th beat (matrix column 15) w/ corresponding illuminated note on left
*Button on testPin used to play note that cursor is on for reference [testNote()]
*Overscrolling backwards enabled (down to count = -6) to reach beat 1 w/ cursor
*SWBpin button used to place notes - modifies compBuffer [placeNote()]
  >Checks for room in compBuffer before placing - warns if beat has 5 notes already
  >DOES NOT modify underlying TXT file, so changes disappear after scrolling
  >Can only place notes of length 1

**placeNote() needs modification to accomidate longer notes
**Newly-placed notes must be saved to corresponding TXT file (maybe use additional 48-beat buffer(s) to load entire temp TXT to RAM [increase responsiveness?])
...
void loop() {
  ...
  if((bLast == LOW) && (bState == HIGH)){
    if(digitalRead(REBpinB) == LOW){
      ...
      else if(digitalRead(SWApin) == HIGH && Bcount > 2){
        Bcount--;
      }
    }
    else{
      ...
      else if(digitalRead(SWApin) == HIGH && Bcount < 14){
        Bcount++;
      }
    }
  }
...
void placeNote(){
  ...
  if (Bcount == 14){
    ...
    for (byte g = 0; g < 5; g++){
      if (compBuffer[bi][g][0] == REST){
        if (g < f){
          f = g;
        }
      }
      else if (compBuffer[bi][g][0] == octave[currentOctave][13-Bcount]){
        f = g + 10;
        break;
      }
    }
    if (f == 6){
      ...
      compBuffer[bi][f][0] = octave[currentOctave][13-Bcount];
    }
    else if (f > 6){
      compBuffer[bi][f-10][0] = REST;
    }
  }
  redrawMatrix(compBuffer);
  matrix.setPixel(15, Bcount);
...
void redrawMatrix(int matrixx[12][6][2]){
  ...
  for (int8_t n = startIndex; n < 12; n++){
    for (byte m = 0; m < 6; m++){
      if (m < 5){
        for (int8_t o = 0; o < 12; o++){
          if (matrixx[n-startIndex][m][0] == octave[currentOctave][o]){
            for (byte p = 0; p < ((2*matrixx[n-startIndex][m][1])-1); p++){
              matrix.setPixel(((2*n)+p+1), (13-o));
            }
          }
          if ((matrixx[n-startIndex][m][0] < octave[currentOctave][0]) && (matrixx[n-startIndex][m][0] != REST) && (matrixx[n-startIndex][m][0] != SUSTAIN)){
            matrix.setPixel(23, 15);
          }
          if (matrixx[n-startIndex][m][0] > octave[currentOctave][11]){
